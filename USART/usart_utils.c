...
void enable_usart(USART_TypeDef *USARTx, uint32_t baud_rate) {
	...
	if      (USARTx == USART1) {
		...
		GPIOC->CRH |= GPIO_CRH_CNF10_1 | GPIO_CRH_MODE10 | GPIO_CRH_CNF11_0;
	}
		...
		GPIOC->CRH |= GPIO_CRH_CNF12_1 | GPIO_CRH_MODE12;
...
void disable_usart(USART_TypeDef *USARTx){
	...
	if      (USARTx == USART1) { RCC->APB2RSTR |= RCC_APB2RSTR_USART1RST; }
	else if (USARTx == USART2) { RCC->APB1RSTR |= RCC_APB1RSTR_USART2RST; }
	else if (USARTx == USART3) { RCC->APB1RSTR |= RCC_APB1RSTR_USART3RST; }
#if defined (STM32F10X_HD) || defined  (STM32F10X_CL) || defined  (STM32F10X_XL)
	else if (USARTx == UART4)  { RCC->APB1RSTR |= RCC_APB1RSTR_UART4RST; }
	else if (USARTx == UART5)  { RCC->APB1RSTR |= RCC_APB1RSTR_UART5RST; }
#endif
}
